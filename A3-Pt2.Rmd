...
# Since we interested in data during year 2020,
# we filter the ice_extent dataset so that it only contains data related to 2020
# so that we can work with it later on with covid 2020 dataset
# compare the 2 dataset with months
...
#variable.names(cyclones_data_original)

# Find the average / sum of the hurricane max_wind per year?
# Basin: the region that cyclone happens...
...
cyclones_data_original

# Did covid impact different region? (for feasibility) (one of the aspect that we think)

# Possible Result: We look into the possibility of covid impact hurricanes for different region differently? We look at the graph by faceting, it is actually not. Maybe COVID19 is not the factor of hurricane change.
cyclones_data_original |>
  group_by(Basin, ObservYear) |>
  mutate(AvgMaxWindPerYear = mean(max_wind, na.rm = TRUE)) |>
  select(Basin, ObservYear, AvgMaxWindPerYear, category) |>
  distinct() |>
  ggplot() +
    aes(x = ObservYear, y = AvgMaxWindPerYear) +
    geom_point() +
    geom_line() +
    #geom_smooth(se = FALSE, method = "lm", formula = y ~ x) +
    xlim(2018, 2022) +
...
# Later: do find the result in a 3 months interval...
...
# Ice melt across year 2010 to 2024?
